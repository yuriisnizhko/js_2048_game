...
startButton.addEventListener("click", ()=>{
    if (game.status === "win" || game.status === "lose" || game.status === "playing") {
        game.restart();
...
document.addEventListener("keydown", (evt)=>{
    if (evt.key === "ArrowLeft") game.moveLeft();
    if (evt.key === "ArrowRight") game.moveRight();
    if (evt.key === "ArrowUp") game.moveUp();
    if (evt.key === "ArrowDown") game.moveDown();
    renderBoard(game.getState());
    renderScore(game.getScore());
    renderMessages(game.status);
});
...
function renderMessages(statusValue) {
    ...
    if (statusValue === "win") messageWin.classList.remove("hidden");
    if (statusValue === "lose") messageLose.classList.remove("hidden");
    if (statusValue === "idle") {
 ...
 */ class Game {
   ...
   */ constructor(initialState){
        ...
        this.board = initialState ? initialState.map((row)=>[
                ...row
            ]) // копія переданого стану
         : Array.from({
            length: 4
        }, ()=>Array(4).fill(0)); // порожнє поле
    }
    moveLeft() {
    ...
    verticalMove(direction) {
        if (this.status === "playing") {
            ...
            for(let col = 0; col < 4; col++){
                ...
                if (direction === "up") {
                    for(let row = 0; row < column.length - 1; row++)if (column[row] === column[row + 1]) {
                        column[row] *= 2; // об'єднуємо кубики
                        this.score += column[row]; // оновлюємо рахунок
                        column[row + 1] = 0; // видаляємо об'єднаний кубик
                    }
                } else if (direction === "down") {
                    for(let row = column.length - 1; row > 0; row--)if (column[row] === column[row - 1]) {
                        column[row] *= 2;
                ...
                column = column.filter((num)=>num !== 0);
                if (direction === "up") while(column.length < 4)column.push(0); // додаємо нулі в кінець колонки
                else if (direction === "down") while(column.length < 4)column.unshift(0); // додаємо нулі на початок колонки
                for(let row = 0; row < 4; row++){
                    if (!moved && this.board[row][col] !== column[row]) moved = true; // перевіряємо, чи були зміни
                    this.board[row][col] = column[row]; // оновлюємо колонку на дошці
                }
            }
            if (moved) this.addRandomCube(); // додаємо новий кубик, якщо були зміни
            // Перевірка на виграш
        ...
        if (this.status === "playing") {
            ...
            for(let row = 0; row < 4; row++){
                ...
                if (direction === "left") {
                    ...
                    for(let col = newRow.length - 1; col > 0; col--)if (newRow[col] === newRow[col - 1]) {
                        newRow[col] *= 2;
                ...
                if (direction === "left") while(newRow.length < 4)newRow.push(0); // додаємо нулі в кінець рядка
                else if (direction === "right") while(newRow.length < 4)newRow.unshift(0); // додаємо нулі на початок рядка
                if (!moved && !this.board[row].every((val, idx)=>val === newRow[idx])) moved = true; // перевіряємо, чи були зміни
...
module.exports = Game; // export default Game;
